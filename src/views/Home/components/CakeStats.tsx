...
const CakeStats = () => {
  ...
  if(farms && farms[0] && farms[0].zooPerBlock){
    zooPerBlock = new BigNumber(farms[0].zooPerBlock).div(new BigNumber(10).pow(18)).toNumber();
  }
    ...
    <StyledCakeStats>
      <CardBody>
        <Heading size="xl" mb="24px">
          {TranslateString(534, 'ZOO Stats')}
        </Heading>
        <Row>
          <Text fontSize="14px">ZOO Price</Text>
        ...
        <Row>
          <Text fontSize="14px">{TranslateString(538, 'Total ZOO Burned')}</Text>
        ...
        <Row>
          <Text fontSize="14px">{TranslateString(540, 'New ZOO/block')}</Text>
          <Text bold fontSize="14px">{zooPerBlock}</Text>
        </Row>
      </CardBody>
    </StyledCakeStats>
  )
